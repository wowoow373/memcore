...
class SQLiteManager:
    def __init__(self, db_path: str = ":memory:"):
        self.db_path = self._resolve_writable_db_path(db_path)
    ...
    def _default_db_path() -> str:
        return os.path.join(os.path.expanduser("~"), ".mem0", "history.db")
    ...
    def _resolve_writable_db_path(self, db_path: str) -> str:
        if db_path == ":memory:":
            return db_path

        requested_path = os.path.abspath(os.path.expanduser(db_path))
        fallback_path = os.path.abspath(os.path.expanduser(self._default_db_path()))

        if self._is_db_path_writable(requested_path):
            return requested_path

        logger.warning(
            "History DB path '%s' is not writable. Falling back to '%s'.",
            requested_path,
            fallback_path,
        )

        if self._is_db_path_writable(fallback_path):
            return fallback_path

        raise PermissionError(
            f"Unable to open writable SQLite history DB at '{requested_path}' or fallback '{fallback_path}'."
        )
